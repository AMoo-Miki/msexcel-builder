...
(function() {
  ...
  tool = {
    ...
    d2d: function(d, addOffset) {
      return 25569 + (d.getTime() / 1000 - d.getTimezoneOffset() * 60) / 86400;
    }
  };
  ...
  Sheet = (function() {
    ...
    Sheet.prototype.set = function(col, row, str, type) {
      if ((str != null) && str !== '') {
        if (!this.data[row]) {
          this.data[row] = {};
        ...
        if (str === null || type === 'undefined') {
          str = '';
          type = 'string';
        }

        this.data[row][col].type = type;
        if (type === 'string') {
          if (str) return this.data[row][col].v = this.book.ss.str2id('' + str);
        }
        if (type === 'date') {
          str = (str && tool.d2d(str)) || 0;
        }
    ...
    Sheet.prototype.format = function(col, row, frmt_s) {
      return this.styles['frmt_' + col + '_' + row] = this.book.st.frmt2id(frmt_s);
    };
          ...
          if (ix || (sid !== 1)) {
            ...
            if (ix) {
              if (ix.type === 'string') {
                ...
                if (('' + ix.v).indexOf('=') === 0) {
                  var str = ('' + ix.v.substring(1)).split('~');
                  if (str.indexOf('~') !== -1) {
                    str = str.split('~');
                    c.ele('f', '' + str[0]);
                    c.ele('v', '' + str[1]);
  ...
  Style = (function() {
    ...
    Style.prototype.with_default = function() {
      this.def_frmt_id = this.frmt2id(null);
      ...
      this.def_rotate = '-';
      this.def_wrap = '-';
      this.fill2id({type: 'gray125'});
      return this.def_style_id = this.style2id({
        frmt_id: this.def_frmt_id,
    ...
    Style.prototype.frmt2id = function(frmt) {
      var id, k;
      frmt || (frmt = '-');
      k = 'frmt_' + frmt;
      id = this.cache[k];
      if (id) {
        ...
        this.mfrmts.push(frmt);
        this.cache[k] = this.mfrmts.length + (this.mfrmts.length > 1 ? 165 : 0);
        return this.cache[k];
      }
    };
    ...
    Style.prototype.fill2id = function(fill) {
      ...
      fill.fgTint || (fill.fgTint = '0');
      k = 'fill_' + fill.type + fill.bgColor + '_' + fill.fgColor + '_' + fill.fgTint;
      id = this.cache[k];
      if (id) {
        return id;
      } else {
    ...
    Style.prototype.style2id = function(style) {
      ...
      style.frmt_id || (style.frmt_id = this.def_frmt_id);
      ...
      k = 's_' + style.frmt_id + '_' + style.font_id + '_' + style.fill_id + '_' + style.bder_id + '_' + style.align + '_' + style.valign + '_' + style.wrap + '_' + style.rotate;
      id = this.cache[k];
      if (id) {
        return id;
      } else {
    ...
    Style.prototype.toxml = function() {
      var bders, cs, e, ea, es, fills, frmts, fonts, o, ss, _i, _j, _k, _l, _len, _len1, _len2, _len3, _ref, _ref1, _ref2, _ref3;
      ...
      frmts = ss.ele('numFmts', {
        count: this.mfrmts.length - 1
      });
      _ref = this.mfrmts;
      for (_i = 1, _len = _ref.length; _i < _len; _i++) {
        o = _ref[_i];
        frmts.ele('numFmt', {
          numFmtId:   this.frmt2id(o) - 1,
          formatCode: o//.replace(/([\s,-])/g, "\\$1")
        });
      }
      fonts = ss.ele('fonts', {
  ...
  baseXl = {
    ...
    'xl/styles.xml': '<?xml version="1.0" encoding="UTF-8" standalone="yes"?><styleSheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><fonts count="2"><font><sz val="11"/><color theme="1"/><name val="宋体"/><family val="2"/><charset val="134"/><scheme val="minor"/></font><font><sz val="9"/><name val="宋体"/><family val="2"/><charset val="134"/><scheme val="minor"/></font></fonts><fills count="2"><fill><patternFill patternType="none"/></fill><fill><patternFill patternType="gray125"/></fill></fills><borders count="1"><border><left/><right/><top/><bottom/><diagonal/></border></borders><cellStyleXfs count="1"><xf numFmtId="0" fontId="0" fillId="0" borderId="0"><alignment vertical="center"/></xf></cellStyleXfs><cellXfs count="1"><xf numFmtId="0" fontId="0" fillId="0" borderId="0" xfId="0"><alignment vertical="center"/></xf></cellXfs><cellStyles count="1"><cellStyle name="Normal" xfId="0" builtinId="0"/></cellStyles><dxfs count="0"/><tableStyles count="0" defaultTableStyle="TableStyleMedium9" defaultPivotStyle="PivotStyleLight16"/></styleSheet>'
  };
